...
void TvMenuQuest::InitMenu() {
    LOGI("%s Initialized – Blood Red Theme Active", menuTitle.c_str());
    LoadConfig();
}
...
void TvMenuQuest::DisconnectLobbyGlobal() {
    LOGI("[QUICK ACTION] Emergency Disconnect");
...
void TvMenuQuest::ScrollLeft() {
    ...
    else currentCategoryIndex = static_cast<int>(categories.size()) - 1;
    selectedModIndex = 0;
    ShowNotification("← " + categories[currentCategoryIndex].name);
}

void TvMenuQuest::ScrollRight() {
    if (currentCategoryIndex < static_cast<int>(categories.size()) - 1) currentCategoryIndex++;
...
void TvMenuQuest::ScrollUp() {
    const auto& mods = categories[currentCategoryIndex].mods;
    if (selectedModIndex > 0) selectedModIndex--;
    else selectedModIndex = static_cast<int>(mods.size()) - 1;
}

void TvMenuQuest::ScrollDown() {
    const auto& mods = categories[currentCategoryIndex].mods;
    if (selectedModIndex < static_cast<int>(mods.size()) - 1) selectedModIndex++;
...
void TvMenuQuest::SelectCurrentMod() {
    ...
    if (newState) {
        LOGI("[UI] 🔴 ON [BRIGHT RED #FF0000]: %s", modName.c_str());
        ShowNotification("ON → " + modName);
    } else {
        LOGI("[UI] ⚪ OFF: %s", modName.c_str());
        ShowNotification("OFF → " + modName);
    }
}
...
void TvMenuQuest::RenderUI() {
    ...
    const std::string& currentMod = cat.mods[selectedModIndex];
    bool enabled = IsModActive(currentMod);

    LOGI("%s | [%s] | 🌟 %s | %s",
         menuTitle.c_str(),
         cat.name.c_str(),
         currentMod.c_str(),
         enabled ? "[ON - BRIGHT RED #FF0000]" : "[OFF]");
}

void TvMenuQuest::SaveConfig() {
    std::ofstream file("/data/data/com.AnotherAxiom.GorillaTag/files/tvmenu_config.txt"); // adjust package if needed
    if (!file.is_open()) {
        LOGE("Could not open config for writing");
        return;
    }
    std::lock_guard<std::mutex> lock(stateMutex);
    for (const auto& pair : activeModStates) {
...
void TvMenuQuest::LoadConfig() {
    std::ifstream file("/data/data/com.AnotherAxiom.GorillaTag/files/tvmenu_config.txt");
